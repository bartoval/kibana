...
export const getDiscoverSessionReferences = (data: DiscoverSessionApiData) => {
  ...
  const tabReferences = data.tabs.flatMap((tab) => {
    const [, references] = extractReferences(fromApiTabToSearchSource(tab), {
...
const fromApiTabToDiscoverTab = (apiTab: DiscoverSessionApiTab) => {
  ...
  const tab: DiscoverSessionTab = {
    ...
    label: apiTab.label,
    ...tabFields,
    ...fromApiTabToSessionFields(apiTab),
...
const fromDiscoverTabToApiTab = (tab: DiscoverSessionTab): DiscoverSessionRequestTab => {
  const apiTab = applyTabTypeStateToApiTab(
    {
      id: tab.id,
      label: tab.label,
      ...fromSearchAndTableStateToApiFieldsWithSessionPolicies(tab, tab.serializedSearchSource),
      ...fromSessionTabToApiFields(tab),
    },
    tab.tabTypeState
  );
